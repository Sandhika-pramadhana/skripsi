...
const UserExternalPosSection = ({startDate, endDate} : TanggalType) => {
    ...
    <Card className="w-full col-span-2">
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row w-full">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          <CardTitle>Rekap Data Pengguna Baru</CardTitle>
          <CardDescription>
            Menampilkan jumlah total pengguna baru untuk periode terpilih
          </CardDescription>
        </div>
        <div className="flex">
          <button
      ...
      <CardFooter className="flex-col items-start gap-2 text-sm">
        <div className="flex gap-2 font-medium leading-none">
          Rata-rata pengguna baru dari periode {startDate ? formatDateMonthYear(startDate) : formatDateMonthYear(earliestDate)} - {endDate ? formatDateMonthYear(endDate) : formatDateMonthYear(latestDate)} adalah {formatNumber(averageUser)} pengguna<TrendingUp className="h-4 w-4" />
        </div>
      </CardFooter>
    </Card>
  );
...
export default UserExternalPosSection;

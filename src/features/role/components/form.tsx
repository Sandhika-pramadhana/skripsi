...
const formSchema = z.object({
    roleId: z.string().min(1, { message: "Id harus diisi" }),
...
const FormRole: React.FC<FormRoleProps> = ({
    ...
    const onSubmit = useCallback(async (values : any) => {
        ...
        const res = isEdit && data?.id ? await updateRole({ ...payload, id: data.id }) : await createRole(payload);
      ...
      useEffect(() => {
        if (isEdit && open) {
          form.reset({ roleId: data?.roleId, roleName: data?.roleName});
        }
                    ...
                    <div className="mt-1 px-1">
                        ...
                        <Controller
                            ...
                            <FormItem className="grid grid-cols-4 gap-2 items-center">
                                <FormLabel className="col-span-1 text-left">ID Role</FormLabel>
                                <FormControl>
                                <input {...field} placeholder="Isi id role" className="text-[12px] col-span-3 border rounded-sm p-2 w-full"/>
                                </FormControl>
                        ...
                        <Controller
                            ...
                            <FormItem className="grid grid-cols-4 gap-2 items-center">
                                ...
                                <FormControl>
                                <input {...field} placeholder="Isi nama role" className="text-[12px] col-span-3 border rounded-sm p-2 w-full"/>
                                </FormControl>
                                <FormMessage>{fieldState.error?.message}</FormMessage>
                            </FormItem>
                            )}

...
const preferenceInit: Preference = { columnVisibility: null };
...
export function ListRoles() {
  ...
  const { data: roleManage, isLoading } = useSWR(
  ...
  const tableData = useMemo(() => {
    return ((roleManage?.items as Role[]) || []).map((item) => ({
      id: item.id, 
  ...
  useEffect(() => {
    if (preference.columnVisibility && !preferenceLoadedRef.current) {
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
        ...
        <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Anda tidak memiliki akses untuk melihat data role.
            </p>
          </div>
        </div>
      )}
      ...
      {hasCrudPermissions && (
        <FormRole open={open} onOpenModal={setOpen} isEdit={false} />
      )}
    </main>
  );
}

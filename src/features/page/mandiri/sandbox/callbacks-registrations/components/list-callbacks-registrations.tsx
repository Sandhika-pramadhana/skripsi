...
import { getListCallbacksRegistrationsMandiri } from "@/actions/sandbox/mandiri/callbacks-registrations-mandiri/callbacks-registrations";
import { FilterCallbacksRegistrationsSection } from "./filter";
...
export function ListCallbacksRegistrations() {
  const [preference, savePreference] = useLocalStorage<Preference>(
    "list-callbacks-registrations-preference",
    preferenceInit
  );
  ...
  const statusIdDebounced = useDebounce(statusId, 400);
  ...
  const handleStatusIdChange = useCallback((status: string) => {
    setStatusId(status);
  }, []);
  
  const handleUsernameChange = useCallback((user: string) => {
    setUsername(user);
  }, []); // Added missing username handler
  ...
  const shouldResetPagination = useMemo(
    () => [termDebounced, statusIdDebounced, usernameDebounced, startDate, endDate].join("|"),
    [termDebounced, statusIdDebounced, usernameDebounced, startDate, endDate] // Added usernameDebounced
  );
  ...
  const swrKey = useMemo(() =>
    `list-callbacks-registrations-${termDebounced}-${statusIdDebounced}-${usernameDebounced}-${startDate}-${endDate}-${pagination.pageIndex}-${pagination.pageSize}`,
    [termDebounced, statusIdDebounced, usernameDebounced, startDate, endDate, pagination.pageIndex, pagination.pageSize] // Added usernameDebounced
  );
  
  const { data: apiResponse, isLoading, error } = useSWR(
    swrKey,
    async () => {
      const combinedTerm = [termDebounced, statusIdDebounced, usernameDebounced]
        .filter(Boolean)
        .join(" "); // Include username in search term
      const res = await unwrap(
        getListCallbacksRegistrationsMandiri({
    ...
    },
    {
      ...
      onError: (error) => {
        console.error("Error fetching callbacks registrations:", error);
      },
    }
  );
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold mb-4">Daftar Callbacks Registrations</h1>
      </div>
      {/* Controls */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-2">
          <div className="relative w-96 shrink-0">
            ...
            <Input
              placeholder="Cari callbacks registrations..."
              value={term}
        ...
        <div className="flex items-center gap-4">
          ...
          <FilterCallbacksRegistrationsSection
            startDate={startDate}
            endDate={endDate}
            statusId={statusId}
            username={username} // Added missing username prop
            onDateChange={handleDateChange}
            onStatusIdChange={handleStatusIdChange}
            onUsernameChange={handleUsernameChange} // Added missing username handler prop
...
export default ListCallbacksRegistrations;
...
import { getListLogApis } from "@/actions/sandbox/tsel/log-apis-tsel/log-apis";
import { FilterLogApisSection } from "./filter";
...
export function ListLogApis() {
  ...
  const processNameDebounced = useDebounce(processName, 400);
  ...
  const handleProcessNameChange = useCallback((name: string) => {
    setProcessName(name);
  }, []);
  ...
  const swrKey = useMemo(() => 
    `listLogApis-${termDebounced}-${processNameDebounced}-${startDate}-${endDate}-${pagination.pageIndex}-${pagination.pageSize}`,
  ...
  const { data: apiResponse, isLoading, error } = useSWR(
    swrKey,
    async () => {
      const combinedTerm = [termDebounced, processNameDebounced]
        .filter(Boolean)
        .join(" ");
    ...
    },
    {
      ...
      onError: (error) => {
        console.error("Error fetching log APIs:", error);
      },
    }
  );
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold mb-4">Daftar Log APIs Tsel</h1>
      </div>

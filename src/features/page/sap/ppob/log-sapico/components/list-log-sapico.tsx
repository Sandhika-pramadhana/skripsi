...
import { getListLogSapico } from "@/actions/sap/ppob/log-sapico";

export interface LogSapico {
  id: number;
  nama_file: string;
  response: string;
  tanggal: string;
...
export function ListLogSapico() {
  ...
  const prevTerm = useRef(termDebounced);
  useEffect(() => {
    if (prevTerm.current !== termDebounced) {
      setPagination((prev) => ({ ...prev, pageIndex: 0 }));
      prevTerm.current = termDebounced;
    }
  }, [termDebounced]);
  ...
  const swrKey = useMemo(
    () =>
      `list-log-sapico-${termDebounced}-${pagination.pageIndex}-${pagination.pageSize}`,
    [termDebounced, pagination.pageIndex, pagination.pageSize]
  );
  ...
  const { data: apiResponse, isLoading, error } = useSWR(
    ...
    },
    {
      ...
      onError: (error) => {
        console.error("Error fetching log sapico data:", error);
      },
    }
  );
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold mb-4">Log Sapico</h1>
      </div>

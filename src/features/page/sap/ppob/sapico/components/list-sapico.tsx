...
import { getListSapico } from "@/actions/sap/ppob/sapico";
import { FilterSapicoSection } from "./filter";
...
export function ListSapico() {
  ...
  const flagDebounced = useDebounce(flagFilter, 400);
  const coaDebounced = useDebounce(coaFilter, 400);
  ...
  const handleCoaChange = useCallback((coa: string) => {
    setCoaFilter(coa);
  }, []);
  ...
  const shouldResetPagination = useMemo(
    () => [termDebounced, flagDebounced, coaDebounced, startDate, endDate].join("|"),
    [termDebounced, flagDebounced, coaDebounced, startDate, endDate]
  );
  ...
  const swrKey = useMemo(() =>
    `list-sapico-${termDebounced}-${flagDebounced}-${coaDebounced}-${startDate}-${endDate}-${pagination.pageIndex}-${pagination.pageSize}`,
    [termDebounced, flagDebounced, coaDebounced, startDate, endDate, pagination.pageIndex, pagination.pageSize]
  );
  
  const { data: apiResponse, isLoading, error } = useSWR(
    swrKey,
    async () => {
      const combinedTerm = [termDebounced, flagDebounced, coaDebounced]
        .filter(Boolean)
        .join(" ");
    ...
    },
    {
      ...
      onError: (error) => {
        console.error("Error fetching sapico data:", error);
      },
    }
  );
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between">
        <h1 className="text-3xl font-bold mb-4">Daftar Sapico</h1>
      </div>
      {/* Controls */}

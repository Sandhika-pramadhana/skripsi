...
import { GenerateXmlZDResult } from "@/types/def";

export const createXmlColumns = (): ColumnDef<GenerateXmlZDResult>[] => [
  {
    accessorKey: "date",
    header: "Date",
    cell: ({ row }) => {
      ...
      if (status === 'success') {
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-green-600 font-medium">Posted to SAP</span>
          </div>
        );
      }
      ...
      if (status === 'xml_created') {
        return (
          <div className="flex items-center gap-2">
            ...
            <span className="text-blue-600 font-medium">XML Created</span>
          </div>
        );
      }
    ...
    cell: ({ row }) => {
      ...
      return message ? (
        <span className={`text-sm ${
          ...
          status === 'xml_created' ? 'text-blue-600' : 
          'text-yellow-600'
        }`}>
          {message}
        </span>

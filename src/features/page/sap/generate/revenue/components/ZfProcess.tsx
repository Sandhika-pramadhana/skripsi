...
import { createInsertZFColumns } from "./columns-insert-generic"; 
...
interface ZFRevenueProcessProps {
...
export function ZFRevenueProcess({ 
  insertAction, 
  generateXmlAction 
}: ZFRevenueProcessProps) {
  const [step, setStep] = useState<ProcessStep>('idle');
  const [insertData, setInsertData] = useState<InsertZFResult[]>([]);
  const [xmlData, setXmlData] = useState<GenerateXmlZFResult[]>([]);
  ...
  const insertColumns = useMemo(() => createInsertZFColumns(), []);
  const xmlColumns = useMemo(() => createXmlColumns(), []);

  // ✅ Statistics untuk ZF - HAPUS PPN, GANTI TOTAL GROSS
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      if (response?.results?.length) {
        setInsertData(response.results as InsertZFResult[]);
        setActiveTab('insert');
      } else {
        setError("No data returned from insert operation");
      }
    } catch (err: any) {
      setError(err.message || "Failed to insert ZF data");
      console.error("Error inserting ZF:", err);
    } finally {
      setStep('idle');
    }
  };
  ...
  const handleGenerateXML = async () => {
    ...
    try {
      ...
      ) as GenerateXmlZFResponse;

      if (response?.results?.length) {
        setXmlData(response.results as GenerateXmlZFResult[]);
        setActiveTab('xml');
      } else {
        setError("No data returned from XML generation");
      }
    } catch(err: any) {
      setError(err.message || "Failed to generate XML ZF");
  ...
  const handleProcessBoth = async () => {
    ...
    try {
      ...
      if (insertResponse?.results) {
        setInsertData(insertResponse.results as InsertZFResult[]);
      }
      ...
      if (xmlResponse?.results) {
        setXmlData(xmlResponse.results as GenerateXmlZFResult[]);
        setActiveTab('xml');
      }

      setStep('completed');
    } catch (err: any) {
      setError(err.message || "Failed to process ZF");
      console.error("Error processing ZF:", err);
      setStep('idle');
    }
  };
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">ZF Aggregator Pihak Berelasi</h1>
      </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              {/* ✅ FIXED: MiniCalendar props - SAMBIL ZD & ZE */}
      ...
      {isProcessing && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="text-lg font-medium">
            {step === 'inserting' && "Inserting ZF data to database..."}
              ...
              {insertTable.getRowModel().rows.length ? (
                ...
                <TableRow>
                  <TableCell colSpan={insertColumns.length} className="h-24 text-center">
                    No data. Click Insert ZF to process data.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>

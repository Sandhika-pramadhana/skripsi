...
export function ZzProcess({ 
  insertAction, 
  generateXmlAction 
}: ZZRevenueProcessProps) {
  const [step, setStep] = useState<ProcessStep>('idle');
  const [insertData, setInsertData] = useState<InsertZZResult[]>([]);
  const [xmlData, setXmlData] = useState<GenerateXmlZZResult[]>([]);
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      if (response?.results?.length) {
        setInsertData(response.results as InsertZZResult[]);
        setActiveTab('insert');
      } else {
        setError("No data returned from insert operation");
      }
    } catch (err: any) {
      setError(err.message || "Failed to insert ZZ data");
      console.error("Error inserting ZZ:", err);
    } finally {
      setStep('idle');
    }
  };
  ...
  const handleGenerateXML = async () => {
    ...
    try {
      ...
      if (response?.results?.length) {
        setXmlData(response.results as GenerateXmlZZResult[]);
        setActiveTab('xml');
      } else {
        setError("No data returned from XML generation");
      }
    } catch(err: any) {
      setError(err.message || "Failed to generate XML ZZ");
  ...
  return (
    <main className="p-12 pb-4 border mt-3 rounded-lg shadow-sm bg-white">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-black-700">ZZ COGS BPP DP PC-Imbl Prshn</h1>
        </div>
      </div>
      ...
      {isProcessing && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="text-lg font-medium">
            {step === 'inserting' && `Inserting ZZ COGS data for ${dateRange.length} days...`}
      ...
      {!isProcessing && activeTab === 'insert' && insertStats && (
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4 mb-6">
          <div className="p-6 border-2 border-orange-200 rounded-xl bg-orange-50">
            <p className="text-sm text-gray-600">Total COGS Amount</p>
              ...
              {insertTable.getRowModel().rows.length ? (
                ...
                <TableRow>
                  <TableCell colSpan={insertColumns.length} className="h-24 text-center bg-white">
                    No data. Click Insert ZZ to process COGS data.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>

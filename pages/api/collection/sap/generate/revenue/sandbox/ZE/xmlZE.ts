...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  let connection7: any = null;  // DB8: log_sapico_deposit
  ...
  try {
    ...
    for (const currentDate of allDates) {
      try {
        ...
        const BLART = "ZE";  // ✅ Changed to ZE

        // Get data dari DB6 - ZE
        const querySelect = `
          SELECT * 
          FROM sapico_temp
          WHERE ket = "ZE"  -- ✅ Changed to ZE
            AND DATE(tgl_trx) = ?
            AND flag = 0 
        `;
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <dws:setTransaction xmlns:dws="http://dwssync_pfi_qa" xmlns:xsd="http://dwssync_pfi_qa/xsd">
      <dws:input>`;
        ...
        <xsd:ITEM>
          ...
          <xsd:ZUONR>REVENUE-AGNPK</xsd:ZUONR>  <!-- ✅ Changed to AGNPK -->
          <xsd:KOSTL></xsd:KOSTL>
          <xsd:WWPRD></xsd:WWPRD>
          <xsd:KUNNR>5000000000</xsd:KUNNR>
          <xsd:SGTXT>POSPAY AGEN</xsd:SGTXT>
          <xsd:STATE></xsd:STATE>
        </xsd:ITEM>`;

        // CREDIT ITEM - AGNPK ✅ Different COA from ZD
        xml += `
        <xsd:ITEM>
          <xsd:AUFNR></xsd:AUFNR>
          <xsd:BUKRS>4000</xsd:BUKRS>
          ...
          <xsd:NEWKO>4200210000</xsd:NEWKO>  <!-- ✅ Changed from 4100210000 to 4200210000 -->
          <xsd:WRBTR>${total_sum}</xsd:WRBTR>
          <xsd:GSBER>HO00</xsd:GSBER>
          <xsd:ZFBDT>${startDateFormatted}</xsd:ZFBDT>
          <xsd:ZTERM>Z030</xsd:ZTERM>
      ...
      <dws:type>JSON</dws:type>
        ...
        // Save XML file
        const xmlname = `ZE_Revenue_AGNPB-${startDateFormatted}.xml`;  // ✅ ZE filename
              ...
              if (jsonData[0]?.MESSG === "Document posted successfully") {
                ...
                const queryUpdate = `
                  ...
                  WHERE ket = "ZE"  -- ✅ Changed to ZE
                    ...
                    AND flag = 0
                `;
                await connection6.execute(queryUpdate, [
                  jsonData[0].BELNR,
                  xmlname,
                  currentDate
                ]);
    ...
    return res.status(200).json({
      success: true,
      environment,
      message: `Processed ${results.length} dates (${environment}) - ZE`,
      results,
    ...
    return res.status(500).json({
      success: false,
      error: error.message || 'Failed to generate XML ZE (Sandbox)',

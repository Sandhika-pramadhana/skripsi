...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    <dws:setTransaction xmlns:dws="http://dwssync_pfi_qa" xmlns:xsd="http://dwssync_pfi_qa/xsd">
      <dws:input>`;
        ...
        <xsd:ITEM>
          ...
          <xsd:NEWKO>4100220000</xsd:NEWKO>
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (resultMatch) {
              ...
              // LOG ke DB yang sama (log_sapico_deposit)
              const queryLog = `
    ...
    return res.status(200).json({
      success: true,
      environment,
      message: `Processed ${results.length} dates (${environment}) - ZF`,
      results,
    ...
    return res.status(500).json({
      success: false,
      error: error.message || 'Failed to generate XML ZF (Sandbox)',

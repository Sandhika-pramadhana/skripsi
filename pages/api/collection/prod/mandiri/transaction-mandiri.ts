    ...
    try {
      ...
      if (id) {
        const trx: QueryResult<transaction_mandiri> = await db.query(
          `SELECT
            id, user_id, location_id, transaction_date, category_id, category_name,
            item_type_id, item_type_name, product_id, product_name, estimation,
            payment_type_id, payment_type_name, connote_code, status_id, status_name,
            awb_url, is_bagging, created_at, updated_at, agent_id, account_number,
            posdigi_product_id, bill_amount, fee_amount, ref_id, receipt_number,
            connote_id, payment_status_id, payment_status_name
           FROM transactions
           WHERE id = $1`,
          [id]
        );
        ...
        const itemsQuery: QueryResult<TransactionItem_mandiri> = await db.query(
          `SELECT
            id, transaction_id, weight, length, width, height, diameter,
            value, description, is_insurance, created_at, updated_at
        ...
        const feesQuery: QueryResult<TransactionFee_mandiri> = await db.query(
          `SELECT
            id, transaction_id, fee_amount, insurance_amount, discount_amount,
            fee_tax_amount, insurance_tax_amount, cod_value, total_amount,
      ...
      if (term && typeof term === 'string') {
        ...
        filters.forEach((filter) => {
          if (filter.startsWith('startDate:')) {
            ...
            if (date) {
              allParams.push(date);
              conditions.push(`transaction_date >= $${allParams.length}`);
            }
            ...
            if (amount) {
              allParams.push(parseFloat(amount));
              conditions.push(`fee_amount >= $${allParams.length}`);
            }
        ...
        if (regularSearchTerms.length > 0) {
          ...
          allParams.push(searchPattern);
          conditions.push(`(
            category_name ILIKE $${allParams.length} OR
            id::text ILIKE $${allParams.length} OR
            user_id::text ILIKE $${allParams.length} OR
            product_name ILIKE $${allParams.length} OR
            status_name ILIKE $${allParams.length} OR
            payment_type_name ILIKE $${allParams.length}
          )`);
        }
      }

      allParams.push(validatedPageSize, offset);
      const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
      const totalQuery = `SELECT COUNT(*) AS total FROM transactions ${whereClause}`;
      ...
      const rowsResult: QueryResult<transaction_mandiri> = await db.query(rowsQuery, allParams);
